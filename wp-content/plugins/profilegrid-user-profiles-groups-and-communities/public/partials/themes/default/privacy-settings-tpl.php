...
<form class="pmagic-form pm-dbfl" name="pm_privacy_form" id="pm_privacy_form" method="post">
        ...
        <div class="pm-col">
            ...
            <div class="pm-field-input pm_required">
                <select name="pm_profile_privacy" id="pm_profile_privacy">
                    <option value="1" <?php selected($pmrequests->profile_magic_get_user_field_value($uid,'pm_profile_privacy'),1);?>><?php _e('Everyone','profilegrid-user-profiles-groups-and-communities');?></option>
                    <option value="2" <?php selected($pmrequests->profile_magic_get_user_field_value($uid,'pm_profile_privacy'),2);?>><?php _e('Friends','profilegrid-user-profiles-groups-and-communities');?></option>
                    <option value="3" <?php selected($pmrequests->profile_magic_get_user_field_value($uid,'pm_profile_privacy'),3);?>><?php _e('Group Members','profilegrid-user-profiles-groups-and-communities');?></option>
                    <option value="4" <?php selected($pmrequests->profile_magic_get_user_field_value($uid,'pm_profile_privacy'),4);?>><?php _e('Friends & Group Members','profilegrid-user-profiles-groups-and-communities');?></option>
                    <option value="5" <?php selected($pmrequests->profile_magic_get_user_field_value($uid,'pm_profile_privacy'),5);?>><?php _e('Only Me','profilegrid-user-profiles-groups-and-communities');?></option>
                </select>
            </div>
        </div>
    </div>
    ...
    <div class="buttonarea pm-full-width-container">
      ...
      <input type="submit" value="<?php _e('Submit','profilegrid-user-profiles-groups-and-communities');?>" name="pg_privacy_submit">
    </div>
  </form>
</div>